...
public class NoticeClientController implements Initializable {

    // TODO 이용자 버전의 공지사항 게시판 만들기 - 조회만 되게. (insert, delete, update XXX)
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        tcDate.setCellValueFactory(new PropertyValueFactory<>("notice_chk"));
        ...
        tvNotice.setOnMouseClicked(event -> {
            if (event.getClickCount() > 1) {  // 더블클릭
                ...
                Parent root = null;
                try {
                    root = FXMLLoader.load(getClass().getResource("SelectNoticeClient.fxml"));
                } catch (IOException e) {
                    e.printStackTrace();
                }
